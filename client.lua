...
RegisterNetEvent("Ayse_HUD:eject-other-player-car-client")
AddEventHandler("Ayse_HUD:eject-other-player-car-client", function(velocity)
...
Citizen.CreateThread(function()
    if Config.useKm then speedToKmOrMph = 3.6 else speedToKmOrMph = 2.236936 end
    if Config.seatbeltPlayAlarmSound then
        while true do
            Citizen.Wait(1000)
            local vehicle = GetVehiclePedIsIn(PlayerPed, false)
    ...
    while true do
        ...
        if pedInVeh then
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if IsControlJustReleased(0, Config.seatbeltInput) and vehicleClass ~= 8 then
                    seatbeltIsOn = not seatbeltIsOn
                    if Config.seatbeltPlaySound then
                ...
                if not seatbeltIsOn then
                    if vehIsMovingFwd and (prevSpeed > Config.seatbeltEjectSpeed/2.237) and vehAcc > 981 then
                        SetEntityCoords(PlayerPed, position.x, position.y, position.z - 0.47, true, true, true)
                ...
                if Config.seatbeltDisableExit and seatbeltIsOn then DisableControlAction(0, 75) end
                ...
                if GetPedInVehicleSeat(vehicle, -1) == PlayerPed and GetIsVehicleEngineRunning(vehicle) then
                    if vehIsMovingFwd and (prevSpeed > Config.crashSeatbeltEjectSpeed/speedToKmOrMph) and vehAcc > 784.8 then
                        ...
                        if #seatPlayerId > 0 then TriggerServerEvent("Ayse_HUD:eject-other-player-car", seatPlayerId, prevVelocity) end
                ...
                if GetPedInVehicleSeat(vehicle, -1) == PlayerPed and GetIsVehicleEngineRunning(vehicle) then
                    if vehIsMovingFwd and prevSpeed > Config.crashFlatTire/speedToKmOrMph and vehAcc > 576 then
                ...
                local speedColor = (speed >= Config.speedLimit) and speedColorOver or speedColorUnder
